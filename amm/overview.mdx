...
The CHANSE AMM is a **constant-product automated market maker** that provides trading for tokens that have graduated from the [Launchpad](/launchpad/overview). It uses the standard `x * y = k` formula, where trading fees stay in the pool to benefit liquidity providers.

## How It Works

The AMM maintains pools of two assets: **CHANSE** and a **CW20 token**. The product of both reserves stays constant (minus fees):

```
ansem_reserve * token_reserve = k (constant)
```

When you swap CHANSE for tokens, you add CHANSE to the pool and remove tokens. The price adjusts automatically based on the ratio of reserves.

<Note>
The quote side is set per pool. It defaults to **CHANSE** (`uchanse`), but a pool can instead be quoted in **ANSEM** (`uansem`, the bridged voucher) when the token was launched ANSEM-denominated. The pool math is identical either way; only the native denom on the quote side differs. Note that ANSEM is not a gas token, so fees are still paid in CHANSE.
</Note>
...
When a token graduates (its target market cap is reached):

| Asset | Amount Added to Pool |
|-------|---------------------|
| CHANSE | All curve reserves |
| Token | The reserved 20.69% of supply (20,690 tokens) |
...
```typescript wrap
{
  ...
  ansem_reserve: string;      // CHANSE in pool (uchanse)
