---
title: 'Architecture'
description: 'How ansemchain, the Launchpad, AMM, Bridge, and Telegram Bot fit together'
---

# Architecture

ansemchain is a proof-of-stake blockchain with a set of CosmWasm smart contracts and off-chain services that together form a token launch and trading ecosystem.
...
| Application | ansemchain runtime | v0.53.4 |
...
The Launchpad is a CosmWasm contract that lets anyone create a new token with a built-in bonding curve. It works like pump.fun - tokens start cheap, price rises as people buy, and the token automatically graduates to the AMM when enough CHANSE accumulates.
...
│   CHANSE fee)   │       │   constant-product │       │   dity moves │
...
Each token launches with a fixed total supply of 100,000, split between the bonding curve and AMM liquidity:

| Allocation | Amount | Purpose |
|-----------|--------|---------|
| Tokens on curve | 79,310 | Available for buyers during bonding phase |
| Tokens for LP | 20,690 | Reserved for the AMM pool at graduation |

This split is a fixed constant (`TOKENS_ON_CURVE` / `TOKENS_FOR_LP`): 79.31% on the curve, 20.69% for LP, identical for every token. The oracle CHANSE/USD price sets the graduation raise, not the token split.
...
```
K = virtual_ansem * virtual_tokens              (constant)
virtual_tokens_current = virtual_tokens_start - tokens_sold
virtual_ansem_current    = K / virtual_tokens_current
```

**Spot price** (uchanse per token):
```
price = K * 10^6 / (virtual_tokens_current)^2
```

**Buy calculation** (CHANSE in, tokens out):
```
fee            = ansem_input * buy_fee_bps / 10000
ansem_after_fee  = ansem_input - fee
tokens_out     = virtual_tokens_current - K / (virtual_ansem_current + ansem_after_fee)
```

**Sell calculation** (tokens in, CHANSE out):
```
ansem_gross = virtual_ansem_current - K / (virtual_tokens_current + tokens_in)
fee       = ansem_gross * sell_fee_bps / 10000
ansem_out   = ansem_gross - fee
```
...
| Fee | Rate | Details |
|-----|------|---------|
| Token creation | 80,000 CHANSE | Fixed fee, becomes initial curve reserves |
| Buy fee | 0.5% (50 bps) | Deducted from CHANSE input before curve math |
| Sell fee | 3.5% (350 bps) | Deducted from CHANSE output after curve math |

All fees stay in the curve's CHANSE reserves, which means fees accelerate progress toward graduation. At graduation, all accumulated reserves (including fees) flow into the AMM pool.

### Dynamic Graduation Threshold

The graduation threshold is computed from the current CHANSE/USD oracle price:

```
threshold_uchanse = target_raised_usd * 10^6 / ansem_usd_price
```
...
| Parameter | Default |
|-----------|---------|
| Graduation raise | Dynamic — set by the target market cap below (oracle-based; ≈ 25,700 CHANSE with default parameters) |
| Target starting market cap (USD) | $1,000 |
| Target graduation market cap (USD) | $10,000 |
| Min graduation threshold | 100,000 CHANSE |
| Max graduation threshold | 50,000,000 CHANSE |

The graduation threshold is a fixed CHANSE amount, so it is deterministic regardless of CHANSE price swings. Curve sizing uses constant-product virtual reserves chosen so the AMM pool opens at the same price the curve ends at.

### Graduation Flow

When `ansem_reserves >= graduation_threshold` (auto-triggered on any buy, or callable manually):

1. Mark the curve as graduated (permanently closed)
2. Transfer all remaining unsold tokens to the AMM contract
3. Call AMM `CreatePool` with all CHANSE reserves + remaining tokens
...
| Asset | Amount |
|-------|--------|
| CHANSE | All curve reserves (~graduation threshold) |
| Token | All unsold tokens from the curve (~20,690) |
...
```
pool_value = ansem_reserve * 2
disabled when: pool_value >= initial_ansem * 10
```
...
| Direction | How |
|-----------|-----|
| CHANSE to Token | Send native `uchanse` with `Swap` message to AMM |
| Token to CHANSE | CW20 `Send` to AMM with `SwapTokenForAnsem` inner message |
...
```
1. Creator pays 80K CHANSE → Launchpad creates CW20 token + bonding curve
                                │
2. Users buy/sell on curve      │  price rises as supply decreases
   (0.5% buy fee, 3.5% sell)   │  fees accumulate in CHANSE reserves
                                │
3. CHANSE reserves hit threshold ──┘
                                │
4. Graduation (auto-triggered)  │  all CHANSE + unsold tokens → AMM
                                │
...
## Bridge (Solana ↔ ansemchain)

The bridge is a native chain module (`x/bridge`) with a trusted relayer architecture. It is **multi-asset**: an SPL token on Solana is locked/released, and the corresponding native coin is minted/burned on ansemchain. **CHANSE** (`uchanse`) is the priority asset; **ANSEM** (`uansem`), a 1:1 voucher for the pump.fun ANSEM SPL, is also carried, and more assets can be added. Each asset maps to its own base denom, and gas is always paid in CHANSE regardless of which asset is bridged.

### Solana to ansemchain (Deposit)

```
User locks a bridged SPL token on Solana (CHANSE or ANSEM)
  → Relayer detects lock event
  → Relayer submits MsgMintFromBridge on ansemchain
    - Validates: bridge enabled, caller is relayer, lock not already processed
    - Mints the matching native coin (uchanse / uansem) to recipient
    - Records mint (idempotent dedup by lock ID)
    - Tracks total bridge-minted supply per asset
```

### ansemchain to Solana (Withdraw)
...
User submits MsgBurnForBridgeOut(solana_address, amount)
  ...
  → Relayer releases the matching SPL tokens on Solana
...
| Parameter | Default |
|-----------|---------|
| Max burn per tx | 1,000,000 CHANSE |
| Max mint per tx | 1,000,000 CHANSE |
| Max burns per block | 10 |
| Admin approval threshold | 100,000 CHANSE |
...
```
┌──────────────┐     ┌──────────────────┐     ┌──────────────┐
│   Telegram   │────▶│   Bot Service    │────▶│  ansemchain   │
│   (user)     │     │                  │     │              │
...
```
ansemchain (chain RPC, polled every ~5s)
       │
       ▼
  Block Poller
...
| `GET /api/wallet/balance` | Native CHANSE balance |
| `GET /api/wallet/tokens` | CW20 token balances |
| `POST /api/wallet/broadcast` | Relay signed transactions to chain |

### Oracle Price Feed

An oracle updater fetches the CHANSE/USD price from pump.fun every 5 minutes and submits it to the Launchpad contract. This price is used for USD display values across the apps; graduation itself is denominated in CHANSE and does not depend on the oracle.
...
Handles multi-asset Solana ↔ ansemchain bridging (CHANSE priority, ANSEM, and future assets) with mint/burn mechanics, relayer authorization, and admin approval for large transfers. See [Bridge Architecture](/bridge/architecture) for details.
...
**Flow**: Submit proposal → bridge-verified wallets vote → at threshold, admin seeds AMM pool with CW20 token + CHANSE liquidity.
