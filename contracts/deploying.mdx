...
The `ansem program deploy` command uploads your contract to the chain and creates an instance in a single step.

## Usage

```bash wrap
ansem program deploy <wasm-file> --from <key> [options]
```
...
```bash wrap
# Create init.json
echo '{"count": 42, "owner": "ansem1..."}' > init.json
...
Contract Details:
  Label: my_contract
  Admin: ansem1youraddress...
  Init msg: {}
```

## Deployment Flow

`ansem program deploy` combines two operations:
...
```bash wrap
# 1. Store code only
ansemd tx wasm store artifacts/my_contract.wasm \
  --from alice \
  --gas auto \
  --gas-adjustment 1.5 \
  --chain-id ansem-1 \
...
# 3. Instantiate
ansemd tx wasm instantiate 1 '{"count":42}' \
  --from alice \
  --label "Counter" \
  --admin ansem1... \
  --gas auto \
...
| "insufficient funds" | Not enough uchanse | Fund account |
...
ansem program query ansem1contractaddress... '{"get_count":{}}'
```
