...
The `ansem program build` command compiles your Rust contract to optimized WebAssembly (Wasm).

## Usage

```bash wrap
ansem program build [options]
```
...
<Note>
ansemchain has a maximum contract size of 800 KB. Both methods typically produce contracts well under this limit.
</Note>
...
ansem program deploy target/wasm32-unknown-unknown/release/my_contract.wasm --from alice
```
