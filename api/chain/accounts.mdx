...
Every address on ansemchain has an associated account that tracks its sequence number (nonce), public key, and account number. These are needed for signing transactions.
...
```bash wrap
curl https://rest.ansemchain.fun/cosmos/auth/v1beta1/accounts/ansem1abc123...
```
...
| Field | Description |
|-------|-------------|
| `address` | The bech32-encoded address (prefix `ansem`) |
...
const account = await fetch(
  'https://rest.ansemchain.fun/cosmos/auth/v1beta1/accounts/ansem1...'
).then(r => r.json());
...
```bash wrap
curl "https://rest.ansemchain.fun/cosmos/auth/v1beta1/accounts?pagination.limit=10"
```
