...
curl https://api.ansemchain.fun/api/candles/ansem1cw20contract...

# Last 7 days of daily candles
curl "https://api.ansemchain.fun/api/candles/ansem1cw20contract...?timeframe=1d&from=2026-02-12T00:00:00Z"

# Last 2 hours of 5-minute candles
curl "https://api.ansemchain.fun/api/candles/ansem1cw20contract...?timeframe=5m&from=2026-02-19T08:00:00Z&limit=24"
```
...
| `open` | string | Price at the start of the period (uchanse per token, scaled by 10^6) |
...
| `volume` | string | Total CHANSE trading volume during the period (in `uchanse`) |
| `trade_count` | number | Number of trades during the period |

### Price Conversion

All price values are in `uchanse` per token, scaled by 10^6:

```
"close": "52000"  →  52000 / 10^6 = 0.052 uchanse per micro-token
                  →  0.052 CHANSE per token
```

### Volume Conversion

Volume is in `uchanse`:

```
"volume": "250000000000"  →  250000000000 / 10^6 = 250,000 CHANSE
```
