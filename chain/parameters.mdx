...
description: 'Module parameters configured on ansemchain'
---

# Chain Parameters

ansemchain modules are configured with specific parameters that govern the blockchain's behavior. These parameters can be modified through governance proposals.
...
|-----------|-------|-------------|
| `unbonding_time` | 21 days (1,814,400s) | Time to unbond staked tokens |
...
| `bond_denom` | `uchanse` | Token used for staking |
| `min_commission_rate` | 5% (0.05) | Minimum validator commission |

### Query Staking Params

```bash wrap
ansemd query staking params --node https://rpc.ansemchain.fun
```
...
```bash wrap
ansemd query slashing params --node https://rpc.ansemchain.fun
```
...
| Parameter | Value | Description |
|-----------|-------|-------------|
| `min_deposit` | 10 CHANSE (10,000,000 uchanse) | Minimum deposit for proposal |
...
Proposal Created
       ...
       ▼
  ┌─────────────────┐
  │  Deposit Period │  ◀── 2 days to reach 10 CHANSE
  │    (2 days)     │
  └────────┬────────┘
           │ (deposit reached)
           ▼
...
```bash wrap
ansemd query gov params --node https://rpc.ansemchain.fun
```
...
<Warning>
**Critical:** ansemchain has zero inflation. The mint module is effectively disabled.
</Warning>

| Parameter | Value | Description |
|-----------|-------|-------------|
| `mint_denom` | `uchanse` | Denomination to mint |
...
```bash wrap
ansemd query mint inflation --node https://rpc.ansemchain.fun
```
...
| Parameter | Value | Description |
|-----------|-------|-------------|
| `constant_fee` | 1,000 CHANSE | Fee to trigger invariant check |
...
```bash wrap
ansemd query wasm params --node https://rpc.ansemchain.fun
```
...
```bash wrap
# List all channels
ansemd query ibc channel channels --node https://rpc.ansemchain.fun

# List connections
ansemd query ibc connection connections --node https://rpc.ansemchain.fun
```
...
ansemd tx gov submit-proposal param-change proposal.json \
  --from my-key \
  --chain-id ansem-1
